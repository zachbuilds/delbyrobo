...
public class Communications {
    ...
    static final int teamSecret = 6942069;
    ...
    public boolean broadcastedCreation2 = false;
    public void broadcastFulCenterCreation(MapLocation loc) throws GameActionException {
        if(broadcastedCreation2) return; // don't re-broadcast
        ...
        if (rc.canSubmitTransaction(message, 3)) {
            rc.submitTransaction(message, 3);
            broadcastedCreation2 = true;
        }
    }

    // check the latest block for unit creation messages
    public int getNewFulCenterCount() throws GameActionException {
        int count = 0;
        for(Transaction tx : rc.getBlock(rc.getRoundNum() - 1)) {
            int[] mess = tx.getMessage();
